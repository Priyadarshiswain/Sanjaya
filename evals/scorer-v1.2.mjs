...
export function scoreAnswerV1_2(task, answer, repositoryRoot) {
  ...
  for (const claim of answer.claims) {
    ...
    if (
      ...
      && valueMatchesV1_2(required, claim.value)
    ) {
  ...
  for (const required of task.groundTruth.requiredClaims) {
    ...
    if (singleAnswer && valueMatchesV1_2(required, singleAnswer.value)) {
...
export function valueMatchesV1_2(required, suppliedValue) {
  if (required.matchMode === "citation_only") {
    return true;
  }
  if (required.matchMode === "all_of") {
    return required.acceptedValues.every((group) =>
      (Array.isArray(group) ? group : [group]).some(
        (variant) => canonicalValueMatches(variant, suppliedValue),
      ));
  }
...
export const CLAIM_AMENDMENTS_V1_2 = new Map([
  ["SJ-EVAL-0007:hit_guard", {
    matchMode: "all_of",
    acceptedValues: [["identifier"], ["source"]],
  }],
  ["SJ-EVAL-0007:produce_failure", {
    addAcceptedValues: ["data: undefined", "set to `undefined`", "set to undefined"],
  }],
  ["SJ-EVAL-0008:overlapped_work", {
    addAcceptedValues: ["environment package"],
  }],
  ["SJ-EVAL-0008:failure_policy", {
    addAcceptedValues: ["swallowed", "ignores"],
  }],
  ["SJ-EVAL-0010:child_cleanup", {
    matchMode: "all_of",
    acceptedValues: [["stdin"], ["kill", "kills"]],
  }],
]);
export function amendTaskV1_2(task) {
  const requiredClaims = task.groundTruth.requiredClaims.map((claim) => {
    const amendment = CLAIM_AMENDMENTS_V1_2.get(`${task.id}:${claim.key}`);
    if (!amendment) {
      return claim;
    }
    return {
      ...claim,
      matchMode: amendment.matchMode ?? claim.matchMode,
      acceptedValues: amendment.matchMode
        ? amendment.acceptedValues
        : [...claim.acceptedValues, ...amendment.addAcceptedValues],
    };
  });
  return {
    ...task,
    groundTruth: { ...task.groundTruth, requiredClaims },
  };
}